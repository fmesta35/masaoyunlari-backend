...
(function () {
  ...
  function join(){if(isAuthoritativeChess()||!socket||!socket.connected)return;roomId=getRoomId();if(!roomId)return;if(GV)GV.roomId=roomId;socket.emit('joinRoom',{roomId,userName:getUserName(),maxPlayers:4,gameId:(GV&&GV.currentGame)||window.currentGame||'chess',memberToken:(window.GVAuth&&GVAuth.token?(GVAuth.token()||undefined):undefined)});}
