...
async function main() {
  ...
  // ---- A) Ayni profil, 4 farkli sekme -> Okey masasinda 4 koltuk ----
  {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      await join(s, '1501', 'okey', 'guest:' + PROF + ':tab' + i, 4);
      socks.push(s);
    }
    await sleep(150);
    const room = rooms.get('1501');
    ...
    const listed = await lobbyRoom(BASE, 'okey', '1501');
  ...
  // ---- B) AYNI anahtarla yeni soket = F5/refresh -> rejoin korunur ----
  {
    ...
    await join(a1, '1502', 'okey', KEY, 4);
    let room = rooms.get('1502');
    ...
    await join(a2, '1502', 'okey', KEY, 4);
    await sleep(100);
    // Beklemedeki özel oda boşalınca haritadan SİLİNİR (sunucu tasarımı);
    // aynı anahtarla dönüşte oda taze yaratılır → nesneyi YENİDEN oku.
    room = rooms.get('1502');
    assert.ok(room, 'B: oda mevcut');
  ...
  // ---- D) Kayitli kullanici davranisi DEGISMEDI: ayni 'user:' anahtari rejoin ----
  {
    ...
    await join(d1, '1503', 'okey', 'user:42', 4);
    const d2 = await connect(BASE, 'D-uye-ikinci-cihaz');
    await join(d2, '1503', 'okey', 'user:42', 4);
    await sleep(100);
    const room = rooms.get('1503');
