...
function effectiveClock(room) {
  const gs = room.gameState;
  if (room.gameId !== 'chess') return gs;
  let white = gs.whiteTimeMs, black = gs.blackTimeMs;
  if (gs.status === 'playing' && gs.turnStartedAt) {
    const elapsed = Math.max(0, Date.now() - gs.turnStartedAt);
    if (gs.turn === 'w') white = Math.max(0, white - elapsed); else black = Math.max(0, black - elapsed);
  }
...
io.on('connection', socket => {
  ...
  socket.on('chessMove', data => {
    ...
    let move;
    try { move = chess.move({ from, to, ...(promotion ? { promotion } : {}) }); } catch (_) { move = null; }
    if (!move) {
