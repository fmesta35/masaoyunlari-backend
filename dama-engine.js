...
function play(st,player,from,to){if(st.status!=='playing')return{ok:false,reason:'finished'};if(player!==st.turn)return{ok:false,reason:'not_your_turn'};if(!Array.isArray(from)||!Array.isArray(to))return{ok:false,reason:'bad_move'};const m=allMoves(st).find(x=>x.from[0]===from[0]&&x.from[1]===from[1]&&x.to[0]===to[0]&&x.to[1]===to[1]);if(!m)return{ok:false,reason:hasCapture(st)?'must_capture':'illegal_move'};const p=st.board[from[0]][from[1]];st.board[from[0]][from[1]]=null;st.board[to[0]][to[1]]=p;for(const [r,c] of m.captures){st.board[r][c]=null;st.captures[player]++;}if(p==='r'&&to[0]===0)st.board[to[0]][to[1]]='R';if(p==='b'&&to[0]===7)st.board[to[0]][to[1]]='B';st.history.push({from,to,captures:m.captures});const other=player==='r'?'b':'r';if(!allPieces(st,other)||!allMoves({...st,turn:other}).length){st.status='finished';st.winner=player}else st.turn=other;return{ok:true,move:m,winner:st.winner}}
